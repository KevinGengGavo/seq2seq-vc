...
conf=conf/vtn.tts_pt.v1.ppg_sxliu_V006.yaml

# dataset configuration
src_db_root=./downloads/V006_SS_max_valid    # default saved here
trg_db_root=./downloads/V006_S1_max_valid  # PLEASE CHANGE THIS
dumpdir=dump                                    # directory to dump full features
srcspk=V006_SS_max_valid
...
if [ "${stage}" -le 6 ] && [ "${stop_stage}" -ge 6 ]; then
    ...
    # shellcheck disable=SC2012
    [ -z "${checkpoint}" ] && checkpoint="$(ls -dt "${expdir}"/*.pkl | head -1 || true)"
    outdir="${expdir}/teacher_results/$(basename "${checkpoint}" .pkl)"
    pids=()
    for name in "eval"; do
    (
        ...
        CUDA_VISIBLE_DEVICES="" ${cuda_cmd} JOB=1:${n_jobs} --gpu 0 "${outdir}/${srcspk}_${name}/decode.JOB.log" \
            vc_decode.py \
                --dumpdir "${dumpdir}/${srcspk}_${name}/${npvc_name}/norm_${norm_name}/dump.JOB" \
                --checkpoint "${checkpoint}" \
                --src-feat-type "${npvc_name}" \
                --trg-feat-type "${npvc_name}" \
                --trg-stats "${expdir}/stats.${stats_ext}" \
                ...
                --trg-dumpdir "${dumpdir}/${trgspk}_${name}/${npvc_name}/norm_${norm_name}/dump.JOB"
